...
data = (
    ImageListGray.from_df(df[df.Id != 'new_whale'], 'data/crop_train', cols=['Image'])
...
learn = Learner(
    data,
    CustomPCBNetwork(torchvision.models.vgg16_bn(pretrained=True)),
...
data = (
    ...
    .add_test(ImageItemList.from_folder('data/crop_test'))
    .transform(
        get_transforms(do_flip=False, max_zoom=1, max_warp=0, max_rotate=2),
        size=SZ,
...
print(
    'new_whales at 1st pos:',
    pd.read_csv(f'subs/{name}.csv.gz')
    .Id.str.split()
    .apply(lambda x: x[0] == 'new_whale')
    .mean(),
)
if SAVE_TEST_MATRIX:
    print('Saving test feats')
    test_shortlist = get_shortlist_fnames_test(
        distance_matrix_idxs_test, cm3, df, learn, []
    )
    torch.save(
        {
            'test_feats': test_feats.detach().cpu(),
